// components/swipe-cell/swipe-cell.js
// TODO 会对父组件有副作用，需要调研一下
Component({
 /**
  * 组件的属性列表
  */
 ...
 methods: {
  ...
  handleTouchStart(e) {
   ...
   this.setData({
    ...
    this.startX = this.data.translateX || 0 // 组件初始位置
    this.direction = null // 记录手指滑动方向 X:左右滑动； Y:上下滑动
    this.actionWidth = 30;
    console.log(this.touchStartX, this.touchStartY, this.startX, this.data.translateX)
   });
   e.stopPropagation()
  },
  ...
  handleTouchMove: function (e) {
   ...
   console.log(this.moveX, this.actionWidth, 'action')
   ...
   if ((this.startX === 0 && this.moveX > 0) || (this.startX === -this.actionWidth && this.moveX < 0)) {
    ...
    console.log('qita', this.touchMoveX - this.touchStartX, this.startX)
    this.setData({
